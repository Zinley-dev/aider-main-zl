...
async def handle_file_extraction(request, response: str, io, coder):
    ...
    # Original logic for handling non-SEARCH/REPLACE responses
    print("🔍 No SEARCH/REPLACE blocks found, using original file extraction logic")
    ...
    # LUÔN force write file nếu có target file
    if target_file:
...
def create_temp_repo(files: List[str] = None) -> str:
    ...
    # Nếu không có files, dùng mặc định ["index.json"]
    if not files:
        files = ["index.json"]
